...
//     const normalization = str.toLowerCase().replace(/[^a-z0-9]/g, "");
//     const reversed = normalization.split("").reverse().join("")
//     if(normalization === reversed){
//         return true
//     }
...
const isPalindrome = (str)=>{
    const normalization = str.toLowerCase().replace(/[^a-z0-9]/g, "");
    let left = 0;
    let right = normalization.length -1;

    while(left < right){
        if(normalization[left] === normalization[right]){
            return true
        }
